...
export default class CrazyApp extends QdkApp {
  constructor(options: QdkAppOptions) {
    ...
    this.hook('after:synth', async () => {
      await PackageManager.required(subproject).pkgRun('run build');
      await PackageManager.required(crazyproj).pkgRun('run build');
    });
...
export { PackageJsonOptions } from '../../src/index.mjs';
// ---- FOR TESTS: end ----
