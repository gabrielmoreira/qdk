...
export class ReactNativeAppProject extends NodeProject<ReactNativeAppProjectOptionsType> {
  ...
  protected async onAfterSynth(synthOptions: qdk.SynthOptions) {
    const templateGenerated = await this.generateReactNativeAppTemplate();
    if (templateGenerated) {
      // Resynth the project files (workaround until we change the way we generate)
      await this.synth(synthOptions);
      ...
      const startCommand = qdk.chalk.bold.yellow(
        this.packageManager.type === 'pnpm'
          ? `pnpm --filter ${this.name} start`
          : `yarn workspace ${this.name} start`,
      );
      this.log(qdk.chalk.green(`Run [${startCommand}] to start react native`));
    }
    await super.onAfterSynth(synthOptions);
  }
  ...
  protected async generateReactNativeAppTemplate() {
    ...
    const rnVersion = this.options.packageJson?.dependencies?.['react-native'];
    if (rnVersion) {
      args.push('--version', rnVersion);
    }
    args.push('--skip-git-init');
    args.push('--skip-install');
    args.push('--replace-directory true');
    const cmd = args.join(' ');

    // Generate the react native app template
    await this.runCmd(cmd, { cwd: this.root.project.options.cwd });

    if (this.packageManager.type === 'pnpm') {
    ...
    await this.deleteFiles('del-cli', '.eslintrc.mjs', '.gitignore');
