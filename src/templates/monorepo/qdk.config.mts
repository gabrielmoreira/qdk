...
// Import components from the local './qdk/components/index.mjs' (it's not an external library)
...
import * as myapp from './.qdk/components/index.mjs';
...
export default class MyApp extends qdk.QdkApp {
  constructor(options: qdk.QdkAppOptions) {
    ...
    const usePnpm = true; // set to false if you prefer yarn
    ...
    const monorepo = new myapp.MonorepoProject(this, {
      ...
      version: '0.1.0',
      packageManager: usePnpm ? 'pnpm' : 'yarn',
      npmrc: usePnpm
        ? {
        ...
        : undefined,
      // yarn rc file is only created if package manager is 'yarn'
      yarnrc: !usePnpm
        ? {
            // Configure yarn for React Native projects
            // More info:
            // - https://yarnpkg.com/getting-started/qa#how-easy-should-you-expect-the-migration-from-classic-to-modern-to-be
            nodeLinker: 'node-modules',
            // - https://yarnpkg.com/migration/guide#replace-nohoist-by-nmhoistinglimits
